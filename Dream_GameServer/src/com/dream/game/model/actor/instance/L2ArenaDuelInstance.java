...
public class L2ArenaDuelInstance extends L2NpcInstance
{
	...
	private void tournamentHourWindow(L2PcInstance player)
	{
		NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());

		html.setFile("data/html/mods/ArenaDuel/10006-7.htm");
		html.replace("%objectId%", String.valueOf(player.getTargetId()));
		html.replace("%StartHourA%", String.valueOf(Config.START_HOUR_A));
		html.replace("%EndHourA%", String.valueOf(Config.END_HOUR_A));
		html.replace("%StartHourB%", String.valueOf(Config.START_HOUR_B));
		html.replace("%EndHourB%", String.valueOf(Config.END_HOUR_B));
		player.sendPacket(html);
	}
