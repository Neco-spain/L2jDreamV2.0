		...
		if (command.startsWith("admin_setbuffshopslot"))
		{
			...
			else
			{
				...
				if (value >= 0)
				{
					...
					admin.sendMessage((new StringBuilder()).append("Changed ").append(((L2PcInstance) admin.getTarget()).getName()).append("'s buffshop limit to ").append(value).append(".").toString());
